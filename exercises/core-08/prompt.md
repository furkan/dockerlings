...
3.  **Create a table** using `docker exec` to run the `psql` client inside the container. Create the table in the default `postgres` database. The table should be named `dvd_rentals`.

4.  **Insert a row** into your new table in the default `postgres` database. The row should contain the title `The Grand Budapest Hotel`.
...
4.  **Verify your data**: `exec` into this new container and run `SELECT * FROM dvd_rentals;` in the default `postgres` database. You will see the movie title you added earlier has reappeared!

### Final State

To complete the exercise, the checker needs to find the `pgdata` directory containing the default `postgres` database with the `dvd_rentals` table and the correct row. You can **leave the container running or stopped**, as the checker will only inspect the `pgdata` directory itself.
